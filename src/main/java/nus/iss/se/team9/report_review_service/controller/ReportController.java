...
public class ReportController {
	...
	@Autowired
	public ReportController(ReportService reportService, RecipeService recipeService, UserService userService,
	...
	public ResponseEntity<List<MemberReport>> getMemberReportsByMemberReported(
	...
	public ResponseEntity<RecipeReport> approveRecipeReportById(@RequestParam("id") Integer id) {
		try {
			...
			return approvedReport.map(ResponseEntity::ok)
								 .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND)
																.body(null));
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	public ResponseEntity<String> reportMember(@RequestBody ReportMemberRequest request,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
								 .body("Error reporting member: " + e.getMessage());
		}
	}
	...
	public ResponseEntity<Map<String, Integer>> getRecipeIdByRecipeReportId(@RequestParam("id") Integer reportId) {
		try {
			...
			if (recipeReport != null && recipeReport.getRecipeReported() != null) {
				Integer recipeId = recipeReport.getRecipeReported()
											   .getId();
